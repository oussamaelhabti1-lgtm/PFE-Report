...
\begin{table}[H]
	...
	\begin{tabular}{>{\centering\arraybackslash}p{1.5cm} p{4cm} p{7cm}}
		...
		\rowcolor{tablerowalt}
		\textbf{5} & Détection Active Snort IDS & Valider la détection des attaques Telnet, DVWA et mouvement latéral via signatures IDS \\
...
Objectif : déployer une infrastructure segmentée (WAN/DMZ/LAN) sous VMware Workstation avec pfSense, pfBlockerNG, UFW et Fail2Ban pour établir une base défensive mesurable.

À l'issue de ce chapitre, l'étudiant maîtrise le déploiement VMware segmenté, la configuration pfSense (routage inter-zone, NAT, règles pare-feu), le filtrage pfBlockerNG, la sécurité hôte (UFW, Fail2Ban, Docker) et l'analyse des logs firewall.
	...
	\begin{itemize}
		...
		\item Sécurité hôte renforcée via UFW, Fail2Ban et conteneurisation.
		\item Validation opérationnelle par analyse des journaux firewall.
	\end{itemize}
	
	Les tests de connectivité confirment que les services exposés (HTTP, SMB) restent accessibles depuis le WAN avec des règles maîtrisées. Les logs ont permis de corriger des erreurs critiques (typo NAT, mismatch firewall) avant la phase de reconnaissance du Chapitre~\ref{chap:reconnaissance}.
...
\section{Conclusion}
	...
	DVWA reste \textbf{NON SÉCURISÉE} pour tout environnement de production : les vulnérabilités identifiées permettent une compromission rapide de l'application et du serveur. Les traces générées par ces attaques sont corrélées au Chapitre~\ref{chap:snort} pour vérifier la capacité de détection IDS.
...
\section{Introduction et Objectifs}
	...
	Cette Phase 5 transforme les preuves offensives en validation défensive. pfSense contrôle les flux L3/L4, tandis que Snort inspecte le \textit{payload} (DPI) pour qualifier les scans, exploitations et mouvements latéraux documentés dans les phases précédentes.
	
	\subsection{Objectifs Pédagogiques}
	
	À l\'issue de cette Phase 5, l'étudiant doit être capable de :
	...
	Ces validations ferment la chaîne attaque$\rightarrow$détection avant le volet deception (Chapitre~6) et la corrélation SIEM (Chapitre~7).
	
	% ==============================================================================
	% FIN DU CHAPITRE 5 — SNORT IDS
	% ==============================================================================
...
Cette annexe regroupe les commandes de référence, extraits de configuration, scripts, glossaire et ressources utilisés tout au long des chapitres 0 à 7 (Introduction Générale + chapitres numérotés 1 à 7). Elle constitue une référence technique rapide pour reproduire ou auditer les environnements de laboratoire.
\end{tcolorbox}
