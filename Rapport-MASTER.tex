...
\section{Introduction et Objectifs Pédagogiques}
	...
	\subsection{Contexte du Projet}
	Cette phase s\'inscrit dans la continuité de la Phase 1 et se concentre sur la dimension \textbf{offensive} de la cybersécurité. L'objectif est d'adopter la perspective de l'attaquant pour évaluer la \textbf{surface d'attaque} exposée et quantifier l'efficacité des contrôles de sécurité mis en place lors de la Phase 1.
	
	Cette phase utilise des outils de reconnaissance active (Nmap) pour cartographier les services exposés, démontre l'utilisation du NAT (Network Address Translation) avec Port Forwarding pour contrôler l'exposition des services, et analyse les résultats pour quantifier la réduction de la surface d'attaque obtenue.
	...
	pfSense (Phase 1) et les tests de validation préliminaires (Phase 2). Parrot Security est 
	...
	\subsubsection{Notes sur l'Architecture}
	
	\paragraph{Machine Administrative (CentOS 7)}
	...
	\begin{itemize}
		...
		\item \textbf{Usage Phase 1:} Configuration initiale de pfSense via interface web
		\item \textbf{Usage Phase 2:} Tests de validation initiaux (client Telnet disponible nativement)
	\end{itemize}
	...
	\paragraph{Machine Attaquante (Parrot Security)}
...
\section{Introduction et Contexte Opérationnel}
	...
	\subsubsection{Progression Pédagogique des Trois Phases}
	
	Les trois phases du projet forment une progression logique reproduisant un scénario réaliste de test d'intrusion :
	...
	\begin{table}[H]
		...
		\begin{tabular}{>{\centering\arraybackslash}p{1.5cm} p{4cm} p{7cm}}
			...
			\hline
			\rowcolor{tablerowalt}
			\textbf{Phase 1} & Infrastructure Défensive & Déploiement de pfSense, segmentation réseau (WAN/DMZ/LAN), configuration des règles de pare-feu, mise en place de Snort IDS/IPS et pfBlockerNG \\
			
			\textbf{Phase 2} & Configuration \& Reconnaissance & Installation de services exposés (Telnet, HTTP, SMB), configuration du port forwarding NAT, reconnaissance active avec Nmap, analyse de la surface d'attaque \\
			
			\rowcolor{tablerowalt}
			\textbf{Phase 3} & Exploitation \& Post-Exploitation & Compromission du serveur DMZ, établissement de persistence, pivoting vers le LAN, mouvement latéral, exfiltration de données \\
			\hline
		\end{tabular}
		\caption{Progression des phases du projet Phase 1-Phase 2-Phase 3}
		\label{tab:tp-progression}
	\end{table}
	
	\subsubsection{Rappel des Acquis des Phases Précédentes}
	
	\paragraph{Phase 1 : Fondations Défensives}
	La Phase 1 a établi l'infrastructure de base :
	\begin{itemize}
	...
	\paragraph{Phase 2 : Préparation du Terrain d'Attaque}
	La Phase 2 a configuré les services cibles et réalisé une reconnaissance :
	\begin{itemize}
		\item \textbf{Installation du service Telnet} sur le serveur Ubuntu DMZ (Chapitre 2 de la Phase 2).
		\item \textbf{Configuration des règles NAT} pour exposer les ports 23, 80, 139, 445 (Chapitre 3 de la Phase 2).
		\item \textbf{Reconnaissance active avec Nmap} identifiant les services exposés (Chapitre 4 de la Phase 2).
		\item \textbf{Analyse de la surface d'attaque} révélant les vulnérabilités critiques (Chapitre 5 de la Phase 2).
	\end{itemize}
	
	\textbf{Découvertes Clés de la Phase 2 :}
	\begin{itemize}
	...
	\subsubsection{Positionnement de la Phase 3}
	
	Le \textbf{pivoting} consiste à utiliser un système compromis comme point de rebond (\textit{pivot point}) pour atteindre des réseaux normalement inaccessibles depuis l'extérieur.
	
	\textbf{Cette Phase 3 valide de manière pratique les hypothèses de la Phase 2 :}
	\begin{itemize}
		\item Les vulnérabilités théoriques identifiées sont-elles réellement exploitables ?
		\item La segmentation réseau de la Phase 1 peut-elle être contournée via pivoting ?
		\item Quel est l'impact réel d'une compromission de la DMZ sur le LAN ?
	\end{itemize}
	
	\subsubsection{Architecture Cible}
	
	L'infrastructure testée est celle déployée lors de la Phase 1 et configurée lors de la Phase 2 :
	\begin{itemize}
	...
	\begin{enumerate}[leftmargin=*]
		...
		\begin{itemize}
			\item Exploiter les vulnérabilités identifiées lors de la Phase 2 (service Telnet).
	...
	\begin{enumerate}
		\item \textbf{Reconnaissance :} Exploitation des résultats Nmap de la Phase 2 (services exposés identifiés).
	...
	\subsubsection{Corrélation avec les Phases Précédentes}
	...
	\begin{table}[H]
		...
		\begin{tabular}{p{3.5cm} p{5cm} p{4.5cm}}
			...
			\textcolor{tableheadfg}{\textbf{Action Phase 2}} &
			\textcolor{tableheadfg}{\textbf{Action Phase 3}} \\
			\hline
			\rowcolor{tablerowalt}
			...
			\hline
		\end{tabular}
		\caption{Corrélation Phase 2-Phase 3 selon la Cyber Kill Chain}
	...
	L'infrastructure testée est identique à celle déployée lors de la Phase 1. Cette continuité permet de valider l'efficacité (ou les failles) des mécanismes de défense mis en place précédemment.
...
\section{Introduction et Objectifs}
	...
	Cette Phase 4 conclut la progression Phase 1--Phase 3 en basculant du \textbf{mode offensif} (reconnaissance, exploitation, pivoting) vers la \textbf{défense active}. L'objectif est de démontrer qu'un pare-feu périmétrique seul, bien qu'indispensable, ne fournit pas une visibilité suffisante sur les attaques lorsque celles-ci empruntent des flux \textit{autorisés}.  
	
	Dans les phases précédentes, une compromission du serveur DMZ a permis d'initier des actions de mouvement latéral vers le LAN (scan SMB/RDP, tentatives d'exploitation). Le pare-feu pfSense voyait principalement des connexions autorisées entre zones. Snort, au contraire, inspecte le contenu (\textit{payload}) et déclenche des alertes sur des signatures connues.
...
\section{Introduction et Objectifs}
	...
	présentes dans la liste OWASP Top 10 2021 et de valider l'efficacité des techniques de détection et d'exploitation.
...
Les honeypots sont des systèmes intentionnellement vulnérables déployés pour collecter des données sur les attaquants. Contrairement aux systèmes de production que l'on cherche à protéger, un honeypot attire délibérément les attaquants afin d'observer leurs techniques, outils et comportements dans un environnement contrôlé.

Dans le contexte de ce projet, le déploiement de Cowrie constitue la Phase 6 et s'inscrit dans la continuité logique des phases précédentes : après avoir simulé des attaques SSH en Phase 3, nous instrumentons maintenant un honeypot pour capturer automatiquement ce type d'activité malveillante.
...
Ce chapitre a présenté le déploiement et l'utilisation de Cowrie comme outil de collecte de renseignements sur les attaquants. Les honeypots constituent un complément précieux aux systèmes IDS/IPS traditionnels : là où Snort détecte les attaques contre les systèmes réels, Cowrie les attire délibérément dans un environnement contrôlé, permettant une analyse approfondie des techniques et outils utilisés.

Les données collectées par Cowrie alimenteront directement le SIEM Wazuh (Phase 7), permettant une corrélation cross-phase et la détection de chaînes d'attaque multi-étapes.
...
Une SIEM (\textit{Security Information and Event Management}) fédère les données de sécurité provenant de multiples sources pour permettre une analyse centralisée, une détection d'anomalies et une réponse aux incidents coordonnée. Dans notre contexte, Wazuh agrège les événements des six phases précédentes pour offrir une vision unifiée de la sécurité de l'infrastructure.
...
Ce chapitre a présenté l'intégration de Wazuh comme plateforme SIEM centrale de notre infrastructure de sécurité. En agrégeant les données des sept phases du projet, Wazuh offre une visibilité complète sur la chaîne d'attaque, de la reconnaissance initiale (Phase 2) jusqu'à la post-exploitation (Phase 3), en passant par la détection IDS (Phase 4), les attaques web (Phase 5) et les sessions honeypot (Phase 6).

Les perspectives d'évolution incluent l'intégration d'une plateforme SOAR (\textit{Security Orchestration, Automation and Response}) pour automatiser davantage la réponse aux incidents, ainsi que l'adoption d'une architecture Zero Trust pour renforcer la posture de sécurité globale.
